...
# 지도파일 저장경로: /data/geo/kostat_census_2012
# 등고면: BAS_CNTR_PG.shp
# 등고선: BAS_CNTR_LS.shp
# 시도군구: BND_SIGUNGU_PG.shp
...
# 데이터로딩하기
path.map <- "/data/geo/kostat_census_2012"
map.filename.shape <- paste(path.map,"/", "BND_SIGUNGU_PG.shp", sep="", collapse="")
map.filename.dbf <- paste(path.map,"/", "BND_SIGUNGU_PG.dbf", sep="", collapse="")
...
xy <- geocode("서울특별시 서초구 서초동 1321-6")  # 넥스알 주소
xy <- geocode("제주시")  # 제주시
