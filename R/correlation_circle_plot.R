...
circle.corr <- function(corr, col=c("black","white"), bg = "white", cex = 1, order = FALSE, title = "", ...) {
  if (is.null(corr))
    return(invisible())
  ...
  text(1:m, rep(n + 1 + ylabwidth/2, m), cname, srt = 90, col = "red", cex = cex)
  title(title)
  ...
  symbols(rep(1:m, each = n), rep(n:1, m), add = TRUE, inches = F, circles = as.vector(sqrt(abs(corr))/2), bg = as.vector(bg))
}
## examples
data(mtcars)
